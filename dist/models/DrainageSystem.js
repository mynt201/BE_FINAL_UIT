...
const drainageSystemSchema = new mongoose_1.default.Schema({
    ward_id: {
        type: String,
        ref: 'Ward',
...
drainageSystemSchema.methods.getCapacityUtilization = function () {
    if (!this.current_flow_rate || !this.design_capacity)
        return 0;
...
drainageSystemSchema.methods.calculateEfficiency = function () {
    ...
    const unresolvedBlockages = this.blockages.filter((b) => !b.resolved).length;
    baseEfficiency -= unresolvedBlockages * 5;
    return Math.max(0, Math.min(100, baseEfficiency));
};
drainageSystemSchema.statics.getByWard = function (wardId) {
    return this.find({ ward_id: wardId, is_active: true });
};
drainageSystemSchema.statics.getSystemStats = async function (wardId) {
...
exports.default = mongoose_1.default.model('DrainageSystem', drainageSystemSchema);
//# sourceMappingURL=DrainageSystem.js.map