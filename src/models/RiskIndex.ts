...
const riskIndexSchema = new mongoose.Schema<IRiskIndexDocument>(
  {
    ward_id: {
      type: String,
      ref: 'Ward',
...
riskIndexSchema.statics.getLatestByWard = function (wardId, period = null) {
  const query: any = { ward_id: wardId, is_active: true };
  if (period) {
    query.period = period;
  }
...
riskIndexSchema.statics.getRiskDistribution = async function (startDate, endDate, period = null) {
  ...
  if (period) {
    (matchStage as any).period = period;
  }
...
riskIndexSchema.statics.getRiskTrends = async function (
  wardId,
  startDate,
  endDate,
  period = 'daily'
...
riskIndexSchema.methods.calculateTrend = async function () {
  ...
  const previousRecord = await (RiskIndex as typeof mongoose.Model)
    .findOne({
...
export default mongoose.model<IRiskIndexDocument, IRiskIndexModel>('RiskIndex', riskIndexSchema);
