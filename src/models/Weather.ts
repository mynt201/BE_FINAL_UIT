...
const weatherSchema = new mongoose.Schema<IWeatherDocument>(
  {
    date: {
      type: Date,
...
weatherSchema.virtual('rainSeverity').get(function () {
  if (this.rainfall >= 100) return 'extreme';
  if (this.rainfall >= 50) return 'heavy';
  if (this.rainfall >= 25) return 'moderate';
  if (this.rainfall >= 5) return 'light';
  return 'none';
});
...
weatherSchema.statics.getByDateRange = function (startDate, endDate, wardId = null) {
  ...
  if (wardId) {
    (query as any)['location.ward_id'] = wardId;
  }
...
weatherSchema.statics.getRainfallStats = async function (startDate, endDate, wardId = null) {
  ...
  if (wardId) {
    (matchStage as any)['location.ward_id'] = wardId;
  }
...
export default mongoose.model<IWeatherDocument, IWeatherModel>('Weather', weatherSchema);
